...
namespace NotasMiUMGWebApp.Controllers
{
    ...
    public class CarrerasController : Controller
    {
        ...
        public async Task<IActionResult> Create([FromBody] Carrera model)
        {
            ...
            if(carrera != null)
            {
                JsonResult resp = Json(new { message = "No se guardó la carrera", error = "Código duplicado" });
                resp.StatusCode = 400;
                return resp;
            }
            ...
            if (carrera != null)
            {
                JsonResult resp = Json(new { message = "No se guardó la carrera", error = "Nombre duplicado" });
                resp.StatusCode = 400;
                return resp;
            }
            try
            {
                ...
                return Ok(new { message = "Carrera guardada" });
            } catch(DbUpdateException ex)
            {
                JsonResult resp = Json(new { message = "No se guardó la carrera", error = ex.InnerException.Message });
                resp.StatusCode = 400;
                return resp;
            }
